"""バッチ16 絶対化1hit第3回 20問を自動抽出。

未レビュー(ai_review なし)で、攪乱肢の絶対化ヒットがちょうど1箇所の問題を
problem_id 順に並べ、F250 より後ろから20問を選ぶ。
...
dst = Path('_work/lecture_batch16_abs.jsonl')
...
candidates = []  # (num, pid, hitcount, hit_choices)
for q in qs:
    if q.get('ai_review'):
        continue
    ...
    if hit_total == 1:  # ちょうど1hit
        candidates.append((pid_num(pid), pid, hit_choices))

candidates.sort()
# F250 より後ろ（num > 250）から20問
after = [c for c in candidates if c[0] > 250]
batch = after[:20]

qs_by_id = {q.get('problem_id'): q for q in qs}
out_lines = []
print(f'1hit未レビュー総数: {len(candidates)} / F250以降: {len(after)}')
print('バッチ16対象:')
